...
export default function AgentDevisPage() {
    ...
    const fetchDocuments = useCallback(async () => {
        // La politique RLS de Supabase s'assure que cet appel ne retourne QUE les documents de cet agent précis.
    ...
    const generatePDF = async (doc: DocumentFinancier) => {
        ...
        try {
            ...
            pdf.text('\u00c9METTEUR (AGENT)', ml + 4, y + 7)
            ...
            drawRow('Sous-total HT', `${doc.sous_total.toLocaleString('fr-FR')}`)
            drawRow('TVA (18%)', `+ ${doc.total_tva.toLocaleString('fr-FR')}`)
            if (doc.remise > 0) drawRow('Remise', `- ${doc.remise.toLocaleString('fr-FR')}`, false, true)
            ...
            pdf.text(`${doc.total.toLocaleString('fr-FR')}`, pw - mr, y + 7.5, { align: 'right' })
            y += 18
            ...
            if (doc.type === 'devis' && y + 28 < ph - 20) {
                ...
                pdf.text('L\'Agent Responsable', sig2X + 4, y + 8)
                pdf.setFont('helvetica', 'bold')
                pdf.setFontSize(6.5)
