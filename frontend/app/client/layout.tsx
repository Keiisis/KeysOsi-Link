...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="h-1 w-28 rounded-full overflow-hidden bg-black/5">
                        <motion.div className="h-full bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full"
                            initial={{ width: '0%' }} animate={{ width: '100%' }}
                            transition={{ duration: 1.8, ease: 'easeInOut' }} />
                    </div>
                    <p className="text-gray-600 text-[10px] uppercase tracking-[0.3em] font-bold">Chargement...</p>
                </div>
            </div>
        )
    }
        ...
        <>
            ...
            <div className="p-5 border-b border-black/[0.06]">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-sm font-black text-[#1a2332]">MON <span className="text-blue-600">ESPACE</span></h1>
                    ...
                    return (
                        <Link key={item.href} href={item.href}
                            className={`group relative flex items-center gap-3 rounded-xl px-3.5 py-2.5 transition-all duration-200 ${
                                isActive ? 'bg-blue-500/10 text-blue-700' : 'text-gray-600 hover:bg-black/[0.04] hover:text-[#1a2332]'
                            }`}>
                            {isActive && (
                                <motion.div layoutId="client-sidebar-active"
                                    className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.6)]"
                                    transition={{ type: 'spring', stiffness: 400, damping: 30 }} />
                            )}
                            <Icon size={17} className={`flex-shrink-0 transition-colors ${isActive ? 'text-blue-700' : 'group-hover:text-blue-600'}`} />
            ...
            <div className="p-3 border-t border-black/[0.06]">
                <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-[#F9FAFB] border border-black/[0.05] mb-2">
                    ...
                    <div className="flex-1 overflow-hidden">
                        <p className="text-xs font-bold text-[#1a2332] truncate">{displayName}</p>
    ...
    return (
        <div className="flex h-screen bg-white text-[#1a2332] overflow-hidden">
            {/* Desktop Sidebar */}
            <aside className="hidden lg:flex w-[240px] h-full flex-col bg-white border-r border-black/[0.06] relative z-40">
                ...
                {mobileOpen && (
                    <>
                        ...
                        <motion.aside
                            ...
                            className="fixed left-0 top-0 h-full w-[260px] bg-white border-r border-black/[0.06] flex flex-col z-[101] lg:hidden">
                            <button onClick={() => setMobileOpen(false)} className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-black/5 text-gray-500">
                                <X size={17} />
                            </button>
                            <SidebarContent />
                        </motion.aside>
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                {/* Topbar */}
                <header className="h-14 flex items-center px-4 lg:px-6 justify-between bg-white/80 backdrop-blur-xl border-b border-black/[0.06] relative z-30">
                    <div className="flex items-center gap-3">
                        <button onClick={() => setMobileOpen(true)} className="p-2 rounded-lg hover:bg-black/5 text-gray-600 lg:hidden">
                            <Menu size={19} />
                        </button>
                        <div className="hidden md:flex items-center gap-1.5 text-xs">
                            <CircleDot size={10} className="text-blue-500" />
                            <span className="font-bold text-gray-600 uppercase tracking-[0.15em] text-[10px]">Espace Client</span>
                            <ChevronRight size={10} className="text-gray-500" />
                            <span className="text-[#1a2332] font-semibold text-[11px]">
                                {NAV_ITEMS.find(i => pathname === i.href || pathname.startsWith(i.href))?.title || 'Navigation'}
                            </span>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button className="relative p-2 rounded-lg hover:bg-black/5 text-gray-600 hover:text-[#1a2332] transition-all">
                            <Bell size={16} />
